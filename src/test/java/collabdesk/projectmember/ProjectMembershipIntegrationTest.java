package collabdesk.projectmember;
...
class ProjectMembershipIntegrationTest {
    ...
    void ownerBuildsProjectTeamAssignsTaskAndRemovalClearsAssignment()
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.assignees[0].projectMemberId")
                        .value(projectMemberId))
                .andExpect(jsonPath("$.assignees[0].displayName")
                        .value("Team Member"));
        ...
        mockMvc.perform(get(tasksUrl).session(owner))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].assignees.length()").value(0));
    }
